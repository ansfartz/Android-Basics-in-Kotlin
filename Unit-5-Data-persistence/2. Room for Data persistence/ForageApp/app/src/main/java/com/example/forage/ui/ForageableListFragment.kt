...
class ForageableListFragment : Fragment() {
    ...
    // 10. Refactor the creation of the view model to take an instance of
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        // 11. observe the list of forageables from the view model and submit it the adapter
        // FIXME: Why "forageableList.let {adapter.submitList(it)} " ?  Why not just "adapter.submitList(forageableList)"
        viewModel.allForageables.observe(this.viewLifecycleOwner, {forageableList ->
            forageableList.let { adapter.submitList(it) }
        })
