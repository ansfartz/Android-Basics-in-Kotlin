...
// 1. pass a ForageableDao value as a parameter to the view model constructor
class ForageableViewModel(
    private val forageableDao: ForageableDao
): ViewModel() {

    // 2. create a property to set to a list of all forageables from the DAO
    val allForageables: LiveData<List<Forageable>> = forageableDao.getForageables().asLiveData()

    // 3. method that takes id: Long as a parameter and retrieve a Forageable from the database by id via the DAO.
    ...
    fun addForageable(
        ...
        // 4. launch a coroutine and call the DAO method to add a Forageable to the database within it
        // FIXME: should Dispatchers.IO be used here? It wasn't added by the exercise
    ...
    fun updateForageable(
        ...
        viewModelScope.launch(Dispatchers.IO) {
            // 5. call the DAO method to update a forageable to the database here
            // FIXME: should Dispatchers.IO be used here? It was already added by the exercise. Why?
            viewModelScope.launch(Dispatchers.IO) {
    ...
    fun deleteForageable(forageable: Forageable) {
        viewModelScope.launch(Dispatchers.IO) {
            // 6. call the DAO method to delete a forageable to the database here
            // FIXME: should Dispatchers.IO be used here? It was already added by the exercise. Why?
...
// 7. view model factory that takes a ForageableDao as a property and creates a ForageableViewModel
