...
class PlantViewModel(application: Application): ViewModel() {
    ...
    internal fun scheduleReminder(
        ...
        val notificationWorkRequest = OneTimeWorkRequestBuilder<WaterReminderWorker>()
            .setInputData(data) // plantName
            .setInitialDelay(duration, unit) // duration and durationUnit until notification shows up
            .build()

        // TODO: Enqueue the request as a unique work request
        val request = workManager.beginUniqueWork(
            "schedule_notification_work",
